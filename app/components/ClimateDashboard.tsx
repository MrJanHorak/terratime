...
function computeYearlyTrends(daily: {
  ...
  for (let i = 0; i < daily.time.length; i++) {
    ...
    if (
      daily.precipitation_sum[i] !== undefined &&
    ...
    .map((yrStr) => {
      ...
      const avgTemp =
        vals.temps.length > 0
          ...
          : 0;
      const totalPrecip =
        vals.precips.length > 0 ? vals.precips.reduce((a, b) => a + b, 0) : 0;
...
export default function ClimateDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'trends' | 'photos' | 'upload'>(
    'trends',
  );
  ...
  useEffect(() => {
    ...
    const fetchFullHistory = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8080/v1/archive?latitude=${location.latitude}&longitude=${location.longitude}&start_date=${EARLIEST_YEAR}-01-01&end_date=${endDateStr}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,wind_speed_10m_max&timezone=auto`,
          { signal: abortController.signal },
        );

        if (response.status === 429) {
          throw new Error(
            'Rate limited by archive API (429) — try again shortly.',
          );
        }
        if (!response.ok) {
  ...
  const trendData: TrendAnalysisResult | null = useMemo(() => {
    ...
    const trends = fullTrends.filter(
      (t) => t.year >= timeframe.start && t.year <= timeframe.end,
    );

    const tempSlope = computeSlope(
      trends.map((t) => ({ year: t.year, value: t.avgTemp })),
    );
    const precipSlope = computeSlope(
      trends.map((t) => ({ year: t.year, value: t.totalPrecip })),
    );
  ...
  const currentWarming = trendData
    ? (trendData.tempSlope * (trendData.endYear - trendData.startYear)).toFixed(
        1,
      )
    : '0.0';
      ...
      <div className='p-4 border-b border-panel-border flex justify-between items-start'>
        <div className='flex flex-col text-left'>
          ...
          <span className='text-[10px] text-gray-500 font-mono mt-0.5'>
            COORD: {location.latitude.toFixed(4)}°N,{' '}
      ...
      <div className='flex-1 overflow-y-auto p-4 flex flex-col gap-4 text-left'>
          ...
          <div className='flex flex-col gap-4'>
            ...
            <div className='flex items-center justify-between text-xs glass-card p-2.5'>
              ...
              <div className='flex items-center gap-1'>
                ...
                >
                  {[1950, 1955, 1960, 1965,  1970, 1975, 1980, 1985, 1990, 1995, 2000, 2005, 2010, 2015, 2020, 2025, 2026].map((y) => (
            ...
            {loading ? (
              <div className='flex flex-col items-center justify-center py-20 text-gray-400 gap-2'>
                ...
                <p className='text-xs'>
                  Fetching and analyzing historical ERA5 records...
                </p>
              </div>
            ) : trendData ? (
              <div className='flex flex-col gap-5'>
                ...
                <div className='grid grid-cols-2 gap-3'>
                  <div className='glass-card p-3 flex flex-col gap-1'>
                    ...
                    <span className='text-xl font-bold text-red-400'>
                      {parseFloat(currentWarming) > 0
                        ? `+${currentWarming}`
                        : currentWarming}
                      °C
                    </span>
                    <span className='text-[9px] text-gray-500 leading-tight'>
                      Warming slope: {(trendData.tempSlope * 10).toFixed(2)}°C
                      per decade
                    </span>
                  </div>
                  ...
                  <div className='glass-card p-3 flex flex-col gap-1'>
                    ...
                    <span className='text-xl font-bold text-blue-400'>
                      {(
                        trendData.precipSlope *
                        (trendData.endYear - trendData.startYear)
                      ).toFixed(0)}{' '}
                      mm
                    </span>
                    <span className='text-[9px] text-gray-500 leading-tight'>
                ...
                <div className='glass-card p-3 flex gap-2.5 items-start bg-blue-950/20 text-xs border-blue-900/40'>
                  ...
                  <p className='text-gray-400 leading-normal'>
                    This linear regression models localized climate variables
                    from Copernicus ERA5 datasets. Slopes measure structural
                    deviations since baseline years.
                  </p>
                </div>
              </div>
            ) : (
          ...
          <div className='flex flex-col gap-4'>
            {localContributions.length === 0 ? (
              <div className='py-12 text-center text-gray-500 text-xs flex flex-col items-center gap-2.5'>
                ...
                <p>
                  No climate observation photos uploaded for this region yet.
                </p>
                <button
                  onClick={() => setActiveTab('upload')}
                  ...
                  <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex justify-between items-center text-[10px] text-gray-400 bg-black/30 p-1.5 rounded-lg border border-panel-border'>
                      ...
                      <span className='font-medium text-gray-300'>
                        Pair {comparisonIndex + 1} of{' '}
                        {Math.floor(localContributions.length / 2)}
                      </span>
                      <button
                        disabled={
                          (comparisonIndex + 1) * 2 >= localContributions.length
                        }
                        onClick={() => setComparisonIndex((prev) => prev + 1)}
                        className='p-1 hover:bg-white/10 rounded disabled:opacity-30'
                      >
                    ...
                    >
                      ...
                      <img
                        src={
                          localContributions[comparisonIndex * 2 + 1]?.imageUrl
                        }
                      ...
                      >
                        <img
                          src={
                            localContributions[comparisonIndex * 2]?.imageUrl
                          }
                        ...
                        <div className='absolute bottom-2 left-2 bg-blue-950/80 border border-blue-500/20 px-1.5 py-0.5 rounded text-[8px] text-blue-300 font-bold uppercase tracking-wider z-20'>
                          {
                            localContributions[comparisonIndex * 2]?.date.split(
                              '-',
                            )[0]
                          }{' '}
                          (Before)
        ...
        {activeTab === 'upload' && (
          <form onSubmit={handleUploadSubmit} className='flex flex-col gap-3'>
            <h3 className='text-xs font-semibold text-gray-300'>
              Contribute Climate Evidence
            </h3>
            <p className='text-[10px] text-gray-500 leading-normal -mt-1'>
              Upload photos showing glacier melts, droughts, deforestation,
              flooding, or successful ecological reclamation. Tied to this
              location: ({location.latitude.toFixed(3)}°,{' '}
            ...
            <div className='grid grid-cols-2 gap-3'>
              <div className='flex flex-col gap-1.5'>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setUploadForm((p) => ({ ...p, date: e.target.value }))
                  }
            ...
            <button
              type='submit'
              disabled={
                uploading || !uploadForm.title || !uploadForm.imagePreview
              }
