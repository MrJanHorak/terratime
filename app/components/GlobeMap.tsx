...
import { Maximize2, Minimize2, MapPin, Layers, Settings, CloudLightning } from "lucide-react";
import { LocationInfo, Contribution, ClimateGridData, ActiveLayers } from "../types";
...
export default function GlobeMap({
  ...
  useEffect(() => {
    ...
    const drawFrame = () => {
      ...
      const temps = climateData.tempGrid[yearKey];
      const precips = climateData.precipGrid[yearKey];
      const winds = climateData.windGrid[yearKey];
      ...
      if (activeLayers.precipitation) {
        ...
        if (precipCtx) {
          ...
          ctx.globalCompositeOperation = activeLayers.temperature ? "source-over" : "copy";
      ...
      if (activeLayers.wind) {
        particles.forEach((p) => {
          ...
          const latIdx = Math.max(0, Math.min(numLats - 1, Math.floor((1 - yPct) * numLats)));
          const lngIdx = Math.max(0, Math.min(numLngs - 1, Math.floor(xPct * numLngs)));
          ...
          } else if ((latVal >= 30 && latVal <= 65) || (latVal <= -30 && latVal >= -65)) {
      ...
      // Trigger map update
      const source = mapRef.current?.getSource("climate-canvas") as mapboxgl.CanvasSource;
      if (source) {
        source.play();
      }
  ...
  useEffect(() => {
    ...
    if (isUploadMode && uploadCoords) {
      const el = document.createElement("div");
      el.innerHTML = '<div class="w-6 h-6 flex items-center justify-center bg-red-500 rounded-full border-2 border-white text-white font-bold text-xs shadow-lg animate-bounce">📍</div>';
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-4 left-4 glass-panel px-3 py-2 text-[10px] text-gray-400 flex items-center gap-2 select-none pointer-events-none">
        ...
        <span>Click any land area to inspect climate metrics & historical trends.</span>
      </div>
    </div>
  );
}
